...
namespace Benguia.Api.Controllers
{
    ...
    public class UberEstimativeRequestsController : Controller
    {
        private readonly UberConfiguration uberConfiguration;

        public UberEstimativeRequestsController(IOptions<UberConfiguration> uberConfigurationOption)
        {
            this.uberConfiguration = uberConfigurationOption.Value;
        }

        [HttpGet, EnableCors("CorsPolicy")]
        public IActionResult Get(string productId, float startLatitude, float startLongitude, float endLatitude, float endLongitude, string token)
        {
            using(var client = new HttpClient())
            {
                ...
                HttpResponseMessage response = client.PostAsync($"requests/estimate", new StringContent(JsonConvert.SerializeObject(new
                {
